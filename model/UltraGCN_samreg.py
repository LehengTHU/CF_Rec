...
class UltraGCN_samreg_Data(Data):
...
class UltraGCN_samreg_RS(AbstractRS):
    ...
    def train_one_epoch(self, epoch):
        ...
        for _, batch in pbar:          
            ...
            users, pos_items, _, _, pos_weight  = batch[0], batch[1], batch[2], batch[3], batch[4]
            ...
            self.model.train()
            mf_loss, cor_loss, reg_loss = self.model(users, pos_items, neg_items, pos_weight)
            loss = mf_loss + cor_loss + reg_loss
            ...
            running_cor_loss += cor_loss.detach().item()
            num_batches += 1
        return [running_loss/num_batches, running_mf_loss/num_batches, running_cor_loss/num_batches, running_reg_loss/num_batches]

class UltraGCN_samreg(AbstractModel):
    ...
    def cal_loss_L(self, users, pos_items, neg_items, omega_weight):
        ...
        neg_scores = (user_embeds * neg_embeds).sum(dim=-1) # batch_size 
    ...
    def cal_loss(self, users, pos_items, neg_items, pop_weight):
    ...
    def forward(self, users, pos_items, neg_items, pop_weight):
        ...
        loss, cor_loss, regularizer = self.cal_loss(users, pos_items, neg_items, pop_weight)

...
class UltraGCN_cause_Data(Data):
...
class UltraGCN_cause_RS(AbstractRS):
    ...
    def train_one_epoch(self, epoch):
        ...
        for _, batch in pbar:          
            ...
            users, pos_items, neg_items, _, _  = batch[0], batch[1], batch[2], batch[3], batch[4]

            max_value = torch.max(users)
            max_value1 = torch.max(pos_items)
            max_value2 = torch.max(neg_items)
            ...
            self.model.train()
            mf_loss, reg_loss, cf_loss = self.model(users, pos_items, neg_items, all_reg, all_ctrl)
            ...
            num_batches += 1
        return [running_loss/num_batches, running_mf_loss/num_batches, running_reg_loss/num_batches, running_cf_loss/num_batches]


class UltraGCN_cause(AbstractModel):
    def __init__(self, args, data) -> None:
        ...
        self.embed_item_ctrl = nn.Embedding(data.n_items, self.emb_dim)
    ...
    def forward(self, users, pos_items, neg_items, all_reg, all_ctrl):
        ...
        all_items=torch.cat([self.embed_item.weight, self.embed_item_ctrl.weight])

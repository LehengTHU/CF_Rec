...
class UltraGCN_macr_RS(AbstractRS):
    ...
    def train_one_epoch(self, epoch):
        ...
        for _, batch in pbar:          
            ...
            users, pos_items, _, _, _  = batch[0], batch[1], batch[2], batch[3], batch[4]
...
class UltraGCN_macr(AbstractModel):
    def __init__(self, args, data) -> None:
        ...
        self.rubi_c = args.c_value * torch.ones([1]).cuda(self.device)
    ...
    def cal_loss(self, users, pos_items, neg_items):
        ...
        self.pos_item_scores =torch.matmul(pos_embeds, self.w.weight)
        self.neg_item_scores = torch.matmul(neg_embeds, self.w.weight)
        self.user_scores = torch.matmul(user_embeds, self.w_user.weight)
        ...
        mf_loss = loss.sum() + self.alpha * mf_loss_item + self.beta * mf_loss_user
